...
\setlength{\hangindent}{56pt}{({\hwfs 大帐}、{\hwfs 堂桌}、{\hwfs 印盒}、{\hwfs  文房}。\textless{}\!{\bfseries\akai 发点}\!\textgreater{}，{\hwfs 四绿}龙套{\hwfs 执月华旗站门}，关羽{\hwfs 上}。夫子盔、黑三、千金、绿靠褶绿蟒、红彩裤、黑厚底靴。{\hwfs 以袖挡脸上}，{\hwfs 至台口中间}）}
...
\setlength{\hangindent}{56pt}{(黄、魏{\hwfs 分坐双门椅}，黄{\hwfs 大边}，魏{\hwfs 小边}。{\hwfs 四白}龙套{\hwfs 站门}，{\hwfs 堂桌}、{\hwfs 文房}、{\hwfs 大座}，{\hwfs 打上}。韩玄纱帽、黪三、缃色蟒或白蟒)}
...
黄忠\hspace{66pt}黄忠、\\魏延\hspace{30pt}\raisebox{5pt}{来也~!}魏延告进!\raisebox{5pt}{({\hwfs 挖进去})参见都督。}

\setlength{\hangindent}{56pt}{韩玄\hspace{30pt}二位将军少礼，请坐。}

黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}谢座。({\hwfs 双跨椅}，黄{\hwfs 大边}，魏{\hwfs 小边})传末将(等)进帐，有何军情议论?}
...
({\hwl 剜萝卜}，{\hwl 钻烟筒}，{\hwl 一扯两扯}，{\hwl 一合两合}，{\hwl 一拉转身}，关{\hwfs 大边}，黄{\hwfs 小边}，{\hwl 架住}，{\hwl 往外一绕两绕三绕}，{\hwl 往里一绕两绕三绕}，关{\hwl 推}黄{\hwl 刀}，{\hwl 打腰封}，\textless{}\!{\bfseries\akai 四击头}\!\textgreater{}{\hwl 亮相}。关{\hwfs 先下}，黄{\hwfs 后下}\footnote{此为北京谭派、汪派打法。上海有``{\bfseries 四门斗}''、\textless{}\!{\bfseries\akai 柳青娘}\textgreater{}唢呐牌子打法。})
...
(黄{\hwfs 上}，关{\hwfs 用拖刀计}，黄{\hwl 抢背落马}，{\hwfs 跪大边}，关{\hwfs 小边举刀亮相}\footnote{{陈超老师介绍:~黄忠落马，念时，关羽始终举刀纹丝不动。}})

\setlength{\hangindent}{56pt}{黄忠\hspace{30pt}(呔，)关羽，俺今落马，为何不杀({\akai 或}:~你为何不斩)?}

\setlength{\hangindent}{56pt}{关羽\hspace{30pt}(黄忠，)关某({\akai 或}:~某家)出世以来，不斩落马之将，回营换马再战。}\footnote{{陈超老师介绍:~}关羽{\akai 念到}``换马再战''，{\hwl 右手一拄刀}，{\hwl 左手慢捋髯}，{\hwfs 很威严}。}

\setlength{\hangindent}{56pt}{(黄{\hwfs 上马}，{\hwl 回身一望}，{\hwfs 下}。关{\hwl 一望两望})}
...
\setlength{\hangindent}{56pt}{(关黄{\hwfs 开打}，{\hwl 一合}，关{\hwfs 小边}，黄{\hwfs 大边}，{\hwl 一拉转身}，{\hwl 架住往里一绕两绕三绕}，{\hwl 往外一绕两绕三绕}，黄{\hwl 压}关{\hwl 刀}，关{\hwl 撤刀}，{\hwl 用刀鐏盖}黄{\hwl 刀头}，{\hwl 鼻子}，{\hwl 削头}。黄{\hwfs 败下}，关{\hwfs 追下})}
...
({\hwfs 四白}龙套{\hwfs 杀死下}。韩玄{\hwfs 抱印下}，魏延{\hwfs 追下}，韩玄{\hwfs 抱印上}，{\hwl 一举两举}，魏延{\hwl 一漫头}，{\hwl 两漫头}，{\hwfs 回身刺}韩玄{\hwfs 倒}，魏延{\hwfs 拿印})
...
\setlength{\hangindent}{56pt}{(魏{\hwl 漫}黄{\hwfs 头}，{\hwfs 欺}黄)}
...
黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}【{\akai 西皮摇板}】黄忠、魏延归降迟。}
...
\setlength{\hangindent}{56pt}{(魏延i{\hwfs 献``彩头''}，关{\hwfs 看}，{\hwfs 手一摆})}
...
黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}谢坐。}
...
\setlength{\hangindent}{56pt}{(({\akai 内白}),搭架子\hspace{20pt}主公({\akai 或}:~主上)到。)}

\setlength{\hangindent}{56pt}{关羽\hspace{30pt}二位将军暂退，}

黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}是。}

\setlength{\hangindent}{56pt}{(黄忠、魏延{\hwfs 下}。黄{\hwfs 先}魏{\hwfs 后})}

\setlength{\hangindent}{56pt}{关羽\hspace{30pt}有请!}

(黄、魏{\hwfs 应是下场门下}。\textless{}\!{\bfseries\akai 吹打\!\textgreater{}{\hwfs 四红}龙套{\hwfs 上``一条鞭''}，诸葛亮、刘备{\hwfs 上}，{\hwfs 挖进门}。刘{\hwfs 坐中}，孔明{\hwfs 大边}，关羽{\hwfs 小边}，{\hwfs 外场椅})
...
黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}参见主公。}

\setlength{\hangindent}{56pt}{刘备\hspace{30pt}少礼，见过先生。}

黄忠\\魏延\hspace{30pt}\raisebox{5pt}{\hspace{30pt}参见先生。}

\setlength{\hangindent}{56pt}{诸葛亮\hspace{20pt}老将军(请起，)后帐歇息。}

\setlength{\hangindent}{56pt}{黄忠\hspace{30pt}谢先生。}

\setlength{\hangindent}{56pt}{（黄忠\textless{}\!{\bfseries\akai 小锣五锤\!\textgreater{}下）}
...
{\bfseries\hwf 第三箭}射中盔缨，又一惊，用手扶住箭走到台中间再拔下箭来看箭起叫头念，龙套两边上}
